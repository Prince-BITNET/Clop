...
patch_spm_checkouts() {
  # Magnet 4.2.0 declares macOS 10.15 but depends on Sauce 2.5.2 which requires
  # macOS 11. Xcode 16+ treats this as a hard error:
  #   "The package product 'Sauce' requires minimum platform version 11.0..."
  # Bump Magnet's manifest to match. Must run AFTER every resolve/checkout.
  MAGNET_PKG="./build/SourcePackages/checkouts/Magnet/Package.swift"
  if [ -f "$MAGNET_PKG" ]; then
    sed -i '' 's|// swift-tools-version:5.1|// swift-tools-version:5.9|' "$MAGNET_PKG"
    sed -i '' 's/.macOS(.v10_15)/.macOS(.v11)/' "$MAGNET_PKG"
    echo "   ✓ Magnet platform patched (macOS 11 for Sauce compat)"
  fi
}
